...
public:
    ...
    //list bufferov, globalna premenna
    ...
    //citacia hlava vo vertex pullery
    struct head_settings {
        BufferID buffer;
        uint64_t offset;
        uint64_t stride;
        AttributeType type;
    ...
    struct vertex_puller_settings_table {
        ...
        struct head_settings head[16];
    };


    //list vertexov, globalna premenna
    struct list_of_vertexes {
        uint64_t number_of_vertexes;
        bool* active_vertexes;
        struct vertex_puller_settings_table * array_of_vertexes;
    }vertex_list;
    ...
    struct program_settings {
        ...
        FragmentShader fs;
        Uniforms uni;
        AttributeType  arr_type[maxAttributes];
    };


    //list program settingov, globalna premenna
    struct list_of_program_settings {
        uint64_t number_of_programs;
        bool* active_programs;
        struct program_settings * array_of_programs;
    }program_list;
    ...
    //list trojuholnikov
    struct triangles_arr {
        uint32_t num_of_triangles;
        struct triangle* arr_of_triangles;
    };


    VertexPullerID activated_ver;
    ProgramID activated_prog;
